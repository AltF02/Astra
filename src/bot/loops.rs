...
use chrono::{DateTime, NaiveDateTime, NaiveTime, Utc, Timelike};
...
async fn check_future_launch(ctx: Arc<Context>) -> Result<(), Box<dyn Error>> {
    ...
    let next_launch = get_next_launch().await?;
    let next_launch = &next_launch.launches[0];
    ...
    let launch = sqlx::query!(
        "SELECT dispatched, timestamp FROM apollo.launches WHERE launch_id = $1",
        next_launch.id
    )
    .fetch_optional(&pool)
    .await?;
    ...
    if let None = launch {
        if launch_stamp < now { // Cannot do in one line
            ...
            let guilds = sqlx::query!("SELECT * FROM apollo.guilds")
                .fetch_all(&pool)
                .await?;
                    ...
                    check_msg(
                        channel.id().send_message(&ctx.http, |m| { m
                            .embed(|e| { e
                                ...
                                .fields(vec![
                                    ("Rocket", format!("➤ Name: **{}**\n➤ Probability of launch: **{}**%", &next_launch.rocket.name, &next_launch.probability), false),
                                    ("Launch Provider",
                                     format!("➤ Name: **{}**\n ➤ Country: **{}**",
